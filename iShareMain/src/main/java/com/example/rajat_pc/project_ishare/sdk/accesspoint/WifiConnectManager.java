...
public class WifiConnectManager extends WifiManagerWrap
{
    ...
    private void handleNetworkStateChanged(NetworkInfo networkInfo)
    {
        if (networkInfo.isConnected() && mWifiConnectListener != null)
        {
            ...
            if (ssid != null)
            {
                if (ssid.contains(SSID_PREFIX))
                    mWifiConnectListener.onWifiConnected();
            }
            else
            {
                ToastUtils.showTextToast(MyApplication.getInstance(), "Your current WiFi has an exception，Please reconnect");
            }
        }
    ...
    /**Stop wifi hotspot, unbind the listener*/
    public void destroy()
    {
    ...
    protected void handleWiFiStateChanged(int wifiState)
    {
        switch (wifiState)
        {
            case WifiManager.WIFI_STATE_ENABLED :
                mWifiManager.startScan();
                handler.postDelayed(runnable, Auto_Scan_Time); // scan WiFi each Auto_Scan_Time
                break;
